                ...
                <div className="w-full grid md:grid-cols-3 grid-cols-2 gap-4">
                    {
                        (posts ?? []).map((post, index) => (
                            <PostWrapper key={post.ID} slug={(post?.title ?? '')?.trim()?.split(' ').join('-')} image={post.iconImage} title={post.title} desc={post.description} />
                        ))
                    }
                </div>
            </div>
            <div className='mt-4 text-end w-full flex items-center justify-end '>
                <Link className='md:px-6 px-2 md:text-base text-sm flex items-center rounded text-[#3f3fc2] transition-all py-[0.4rem] hover:bg-[#e9e9e9ae] bg-[#e9e9e98b]' href={'/blogs'}>
